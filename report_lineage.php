...
// Get view mode and animal type from URL
$view_mode = isset($_GET['view']) ? $_GET['view'] : 'tree';
$animal_type = isset($_GET['type']) ? $_GET['type'] : '';

// Get animal ID if provided - THIS IS THE SELECTED ANIMAL
$animal_id = isset($_GET['id']) ? intval($_GET['id']) : null;
$selected_animal = null;

// Load the selected animal's details if an ID is provided
if ($animal_id) {
    try {
...
// Initialize parent and grandparent containers
$parents = [
    'dam' => null,
    'sire' => null
];
...
// Load parents and grandparents ONLY if we have a selected animal
if ($selected_animal) {
...
// Get breeding stock for the main list view
$breedingStock = [];
try {
    $breedingStockQuery = "
...
// Get potential breeding stock (animals alive but no offspring)
$potentialStock = [];
try {
    $potentialStockQuery = "
...
<?php if (!$selected_animal): ?>
<!-- Breeding Stock List - Only shown when no specific animal is selected -->
<div class="card shadow-sm mb-4">
    <div class="card-header">
...
<!-- Potential Breeding Stock - Only shown when no specific animal is selected -->
<div class="card shadow-sm mb-4">
    <div class="card-header">
